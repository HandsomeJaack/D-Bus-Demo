...
static int method_add(sd_bus_message *m,void *userdata, sd_bus_error *ret_error){
    ...
    char *create = "CREATE TABLE IF NOT EXISTS Programs(program_path TEXT,program_extension TEXT," 
...
/* Returns 0 if success, other value if failure" */
static int method_delete(sd_bus_message *m,void *userdata, sd_bus_error *ret_error){
    (void)ret_error;
    (void)userdata;
	int r;	
	char *path;
...
static int method_variants(sd_bus_message *m,void *userdata, sd_bus_error *ret_error){
    (void)ret_error;
    (void)userdata;
	int r;	
	char *file;
	...
	char *format = strchr(file, '.'); /* Detachment of necessary extension from full file name */
	sqlite3 *db;
    sqlite3_stmt *res;
    ...
    rc = sqlite3_prepare_v2(db, sql, -1, &res, 0); /* Precompilation sql commamd */
    ...
    sqlite3_bind_text(res,1,format,-1,0); /* Adding @format@ paramater to search request */
    char **final = malloc(0);
    int size = 0;
    for(;;){
...
static const sd_bus_vtable daemon_vtable[]={
	...
	SD_BUS_METHOD("Delete", "s", "x", method_delete,SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_METHOD("Variants", "s", "as", method_variants,SD_BUS_VTABLE_UNPRIVILEGED),
	SD_BUS_VTABLE_END
};
